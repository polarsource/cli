...
import type * as Polar from "../services/polar";
 ...
 *
 * Key derivation must match @polar-sh/sdk's `validateEvent`, which does:
 *   base64Secret = Buffer.from(secret, "utf-8").toString("base64")
 *   new Webhook(base64Secret)  // internally: base64.decode(base64Secret)
 *
 * The round-trip means the effective HMAC key is always the raw UTF-8 bytes
 * of the secret string. We replicate that directly here.
 */
...
export const listen = Command.make(
	"listen",
	{
		...
		env: envOption,
		org: orgOption,
		webhookSecret: webhookSecretOption,
	},
	({
		url,
				...
				Effect.async<void, OAuth.OAuthError>((resume) => {
					...
					eventSource.onmessage = (event) => {
						...
						if (Either.isRight(ack)) {
							...
							if (webhookSecret) {
								console.log(
									`  ${dim}Signing${reset}    enabled (--webhook-secret)`,
								);
							}
							console.log("");
						...
						if (Either.isLeft(webhookEvent)) {
							...
							console.error(
								`   ${dim}${JSON.stringify(json).slice(0, 200)}${reset}`,
							);
							return;
						}

						const forwardBody = JSON.stringify(
							webhookEvent.right.payload.payload,
						);
			...
			const listenWithRetry = (
				...
				startListening(token).pipe(
					Effect.catchTag("OAuthError", (error) => {
						...
						console.error(`  ${dim}Reconnecting in ${delaySec}s...${reset}\n`);
